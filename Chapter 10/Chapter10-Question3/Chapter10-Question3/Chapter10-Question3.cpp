...
{
    auto iter = book.insert(inputEntry());

    std::cout << "Added entry for " << iter->first.getName()
            << ". The number is " << iter->second << std::endl;
}
...
{
    Person person{ getPerson() };
    auto iter = book.lower_bound(person);
    if (iter == book.end())
        ...
        auto upper = book.upper_bound(person);
...
void deleteEntry(PhoneBook& book)
{
    ...
    if (iter == book.end())
        ...
        std::vector<std::multimap<Person, string>::iterator> vec;
        for (int i{}; iter != upper; ++iter, i++) {
            vec.push_back(iter);
            std::cout << i << ". " << iter->second << std::endl;
        }
